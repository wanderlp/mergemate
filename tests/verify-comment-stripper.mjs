...
function stripCStyleComments(source) {
  ...
  while (i < n) {
    ...
    if (state === 'line') {
      if (c === '\n') { result += c; i++; state = 'code'; lastSig = c; pendingNewline = false }
      else { i++ }
    } else if (state === 'block') {
      ...
      result += c
      if (c === '\\' && i + 1 < n) { result += c2; i += 2 }
      else if (c === '"') { i++; state = 'code'; lastSig = '"' }
      else { i++ }
    } else if (state === 'sstr') {
      result += c
      if (c === '\\' && i + 1 < n) { result += c2; i += 2 }
      ...
      if (c === '\\' && i + 1 < n) { result += c + c2; i += 2 }
      else if (c === '[') {
        result += c; i++
        while (i < n && source[i] !== ']') {
          if (source[i] === '\\' && i + 1 < n) { result += source[i] + source[i + 1]; i += 2 }
          else { result += source[i]; i++ }
        }
        if (i < n) { result += source[i]; i++ }
      } else if (c === '/') {
        result += c; i++; state = 'code'
        while (i < n && /[gimsuyn]/.test(source[i])) { result += source[i]; i++ }
        lastSig = '/'
      } else {
        result += c; i++
      }
      ...
      if (c === '\n') {
        ...
        pendingNewline = false; i += 2; state = 'line'
      } else if (c === '/' && c2 === '*') {
        pendingNewline = false; i += 2; state = 'block'
      } else if (c === '"') {
        if (pendingNewline) { result += '\n'; pendingNewline = false }
        result += c; i++; state = 'dstr'; lastSig = c
      } else if (c === "'") {
        if (pendingNewline) { result += '\n'; pendingNewline = false }
        result += c; i++; state = 'sstr'; lastSig = c
      } else if (c === '`') {
        if (pendingNewline) { result += '\n'; pendingNewline = false }
        result += c; i++; state = 'tpl'; lastSig = c
      } else if (c === '/' && canStartRegex(lastSig)) {
        if (pendingNewline) { result += '\n'; pendingNewline = false }
        result += c; i++; state = 're'; lastSig = c
      } else {
        if (pendingNewline) { result += '\n'; pendingNewline = false }
        result += c
...
function stripSqlComments(source) {
  ...
  while (i < n) {
    ...
    if (state === 'line') {
      ...
      result += c
      if (c === "'" && c2 === "'") { result += c2; i += 2 }
      else if (c === "'") { i++; state = 'code' }
      else if (c === '\\' && i + 1 < n) { result += c2; i += 2 }
      else { i++ }
    } else {
      if (c === '-' && c2 === '-') { i += 2; state = 'line' }
      else if (c === '/' && c2 === '*') { i += 2; state = 'block' }
      else if (c === "'") { result += c; i++; state = 'sstr' }
      else { result += c; i++ }
    }
  }
  return result
...
function stripLuaComments(source) {
  ...
  while (i < n) {
    ...
    if (state === 'line') {
      ...
      else if (c === '"') { result += c; i++; state = 'dstr' }
      else { result += c; i++ }
    }
  }
  return result
...
function getCommentStyle(ext) {
  if (['js','ts','jsx','tsx','java','kt','kts','c','cpp','cc','cs','h','hpp','hcl'].includes(ext)) return 'c_style'
...
expect('URL con // en string', 'const url = "https://example.com//path"', 'js', 'const url = "https://example.com//path"')
expect('String con // falso comentario', 'const s = "http://not-a//comment"', 'js', 'const s = "http://not-a//comment"')
expect('String con /* falso bloque', 'const s = "/* not a comment */"', 'js', 'const s = "/* not a comment */"')
expect('Comillas simples', `const s = 'http://x.com//y'; `, 'js', `const s = 'http://x.com//y'; `)
expect('Template literal con //', 'const t = `url: https://x.com//y`; ', 'js', 'const t = `url: https://x.com//y`; ')
expect('Escape en string', 'const s = "say \\"hi\\" // not comment"; ', 'js', 'const s = "say \\"hi\\" // not comment"; ')
total += 6
...
expect('i++ / total con comentario después (objeción #1)', 'let ratio = i++ / total;\n// comentario real\nlet z = 3;', 'js', 'let ratio = i++ / total;\nlet z = 3;')
expect('i-- / total con comentario después', 'let ratio = i-- / total;\n// comentario real\nlet z = 3;', 'js', 'let ratio = i-- / total;\nlet z = 3;')
...
expect('SQL con string que contiene -- (CASO REVISOR)', `SELECT 1; -- linea\n/* bloque\nmultilinea */\nSELECT '--not a comment';`, 'sql', `SELECT 1; \n\nSELECT '--not a comment';`)
expect('SQL line comment simple', 'SELECT * FROM t -- comment\nWHERE x = 1', 'sql', 'SELECT * FROM t \nWHERE x = 1')
expect('SQL block comment', 'SELECT /* inline */ * FROM t', 'sql', 'SELECT  * FROM t')
expect('SQL escape de comilla doble', `SELECT 'it''s ok' AS x -- comment`, 'sql', `SELECT 'it''s ok' AS x `)
total += 4

console.log('\n=== Lua: strings y comentarios (objeción revisor) ===')
expect('Lua line comment', 'local x = 1 -- comment\nlocal y = 2', 'lua', 'local x = 1 \nlocal y = 2')
expect('Lua block comment --[[ ]]', 'local x = 1 --[[ comment ]] local y = 2', 'lua', 'local x = 1  local y = 2')
expect('Lua block con == anidado', '--[==[ nested ]==] code', 'lua', ' code')
expect('Lua string con -- dentro', `local s = '--not-comment' -- real`, 'lua', `local s = '--not-comment' `)
expect('Lua double string', `local s = "-- also not --" -- real`, 'lua', `local s = "-- also not --" `)
total += 5
