...
export type StatusInfo =
  ...
  | {
      kind: "image";
      leftDims: ImageDims | null;
      rightDims: ImageDims | null;
    ...
    <div className="flex h-8 items-center gap-3 border-t border-[#3e3e42] bg-[#007acc] px-3 text-sm text-white overflow-hidden">
      ...
      {info.kind === "image" && (
        <>
          {info.leftDims ? (
            <Stat
              label={t("status.leftImage", { w: info.leftDims.width, h: info.leftDims.height })}
            />
          ) : (
          ...
          {info.rightDims ? (
            <Stat
              label={t("status.rightImage", { w: info.rightDims.width, h: info.rightDims.height })}
            />
          ) : (
          ...
          {info.leftDims && info.rightDims && (
            <>
              <Sep />
              {info.leftDims.width === info.rightDims.width &&
              info.leftDims.height === info.rightDims.height ? (
